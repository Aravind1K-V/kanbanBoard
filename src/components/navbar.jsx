...
const NavigationBar = () => {
  const fetchData = useStore(state => state.selectData);
  
  const [isDropdownVisible, setIsDropdownVisible] = useState(false);
  const [currentGroup, setCurrentGroup] = useState(getGroup());
  const [currentOrder, setCurrentOrder] = useState(getOrder());

  const handleSelectionChange = useCallback((e, isGroupSelection) => {
    if (isGroupSelection) {
  ...
  return (
    <div className="padding-left-bottom">
      <div className="displayButton">
        <button className="padding-x-y font-16 btn"
        ...
        {isDropdownVisible && (
          <div className="dropOnClick flex-display-btn padding-x-y">
            <div className="p-3 flex-sb">
              <span>Grouping</span>
              <select value={currentGroup} onChange={(e) => handleSelectionChange(e, true)} className="selectStyle" name="group" id="group">
            ...
            <div className="p-3 flex-sb">
              <span>Ordering</span>
              <select value={currentOrder} onChange={(e) => handleSelectionChange(e, false)} className="selectStyle" name="order" id="order">
                <option value="priority">Priority</option>
