...
<path d="M13.5 2H12.5C11.9477 2 11.5 2.44772 11.5 3V13C11.5 13.5523 11.9477 14 12.5 14H13.5C14.0523 14 14.5 13.5523 14.5 13V3C14.5 2.44772 14.0523 2 13.5 2Z" fill="#5C5C5E"/>
      ...
      default: 
        return (
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M8 1.5L14.5 8L8 14.5L1.5 8L8 1.5Z" stroke="#9E9E9E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        );
    }
  };
  ...
  return (
    selectedData.length > 0 && (
      <div className="dashContainer space-even" >
        {selectedData.map((group, index) => (
          <div key={index} className="dashCardContainer">
            <div className="dashCardHeading flex-sb">
              <div className="leftView">
                {getStatusSVG(group.title)}
                <span>
            ...
            <div className="dashList flex-gap-10">
              {group.value.map((ticket) => {
                const user = allUsers.find(u => u.id === ticket.userId);
                return (
                <Card key={ticket.id} id={ticket.id} title={ticket.title} tag={ticket.tag} available={user ? user.available : false}  />
                );
})}
            </div>
          </div>
